"""
Run the full baseline vs DPML experiment — healthcare treatments dataset.
"""
...
DEFAULT_OUTPUT_DIR = os.path.abspath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "dp_results_healthcare")
)
PLOT_FILENAMES = [
...
def run_experiment(
    *,
    ...
    if verbose:
        pos = float(df["y"].mean())
        _log(
            f"Loaded healthcare frame: {len(df)} rows (subsample cap {pp.get_max_sample_rows()}), "
