...
def build_page_context(output_dir: str, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        "epsilon_why": (
            "On sensitive healthcare analytics, ε controls how much noise (or DP-SGD budget) "
            "you allow while still publishing model quality. "
            "<strong>Smaller ε ⇒ stronger privacy</strong>, often at the cost of "
            "<strong>accuracy / F1</strong> on rare outcomes. "
            "δ (delta) is the small failure probability used with Gaussian mechanisms and Opacus."
        ),
